...
def make_character_index(text=text, characters=CHARACTERS):
    ...
    for line in enumerate(text):
        for character in characters:
            # Check if character has any synonym names.
            if isinstance(character, tuple):
                for synonym in character:
                    if synonym.lower() in line[1].lower():
                        character_index[character[0].lower()].append(line[0])
            elif character.lower() in line[1].lower():
