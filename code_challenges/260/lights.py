...
class LightsGrid:
    ...
    def follow_instructions(self):
        ...
        for instruction in self.instructions:
            ...
            # If 'up' or 'down', need amt to apply
            if instruction[0] == 'up' or instruction[0] == 'down':
                func, amt, s1, _, s2 = instruction
                amt = int(amt)
            else:
                func, s1, _, s2 = instruction
                amt = None
            if func == 'on':
                self.turn_on(s1, s2)
            elif func == 'off':
                ...
                self.turn_up(amt, s1, s2)
            elif func == 'down':
                self.turn_down(amt, s1, s2)
            else:  # func == 'toggle'
                self.toggle(s1, s2)
