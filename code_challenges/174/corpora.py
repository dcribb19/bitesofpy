...
class Corpora:
    ...
    * stopwords
        ...
        * If a list of extract objects were given, remove those too
        ...
        if self.extra:
            for item in self.extra:
                self.txt = self.txt.replace(item, ' ')
        return self.txt
    ...
    def metrics(self) -> List[Tuple[str, int]]:
        ...
        * Using the cleaned up corpus,
          count up how many times each word is used
        * Exclude stop words using STOPWORDS
        * Use count to return the requested amount of the top words,
          defaults to 5

        :return: List of tuples, i.e. ("word", count)
        """
        c = Counter(self.cleaned.split())
    ...
    def graph(self) -> None:
        ...
        * The tag is repeated the number of counts of the word
        ...
        :return: None
        """
        for word in self.metrics:
            print(f'{word[0]:>10} {self.tag * word[1]}')
