...
def operas_both_at_premiere(guest, composer):
    ...
    return (Opera.play for Opera in operas
            ...
            and _get_date(composers[guest].born) < _get_date(Opera.date)
            and _get_date(composers[guest].died) > _get_date(Opera.date)
            and _get_date(composers[composer].died) > _get_date(Opera.date))
