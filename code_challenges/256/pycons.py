...
def get_pycon_events(data=_get_pycon_data()) -> List[PyCon]:
    ...
    # Handle application/ld+json data structure.
    events = soup.find_all('script', {'type': 'application/ld+json'})
    # Load all events as a list of dicts with json.
    events = [json.loads(event.contents[0].strip()) for event in events]
    # Filter for PyCons only.
    pycons = [event for event in events if 'PyCon' in event['name']]
    # Create list of namedtuples.
    return [PyCon(
        pycon['name'],
        pycon['location']['address']['addressLocality'],
        pycon['location']['address']['addressCountry'],
        parse(pycon['startDate']),
        parse(pycon['endDate']),
        pycon['url']
        )
        for pycon in pycons
