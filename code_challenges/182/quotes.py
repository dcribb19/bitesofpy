...
def extract_quotes(html: str = HTML) -> dict:
    ...
    # Find all <p> tags starting with numbers. 
    quotes = re.findall(r'<p>\d+.+</p>', HTML)
    # Split by '"' in  order to get number, quote, author.
    quotes = [quote.split('"') for quote in quotes]
    # Keep quote and split/strip author to get name only.
    quotes = [[x[1], x[2].split('<')[0].strip(' -')] for x in quotes]
