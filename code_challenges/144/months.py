...
def calc_months_passed(year, month, day):
    ...
    try:
        ...
        if new_date.day > MIN_DAYS_TO_COUNT_AS_MONTH:
            months = delta.years * MONTHS_PER_YEAR + delta.months + 1
        else:
            months = delta.years * MONTHS_PER_YEAR + delta.months
        return months
    except ValueError:
        raise ValueError('values must be in proper range')
