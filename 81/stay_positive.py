...
def filter_tweets_on_polarity(tweets, keep_positive=True):
    ...
    return ([tweet for tweet in tweets if tweet.polarity > 0] if keep_positive
            else [tweet for tweet in tweets if tweet.polarity < 0])
...
def order_tweets_by_polarity(tweets, positive_highest=True):
    ...
    return (sorted(tweets, key=lambda x: x.polarity, reverse=True)
            if positive_highest
            else sorted(tweets, key=lambda x: x.polarity)
            )
